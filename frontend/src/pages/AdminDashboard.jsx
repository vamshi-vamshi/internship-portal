...
export default function AdminDashboard() {
  const [tab, setTab] = useState('internships'); // internships | users | applications
  ...
  return (
    <div className="min-h-screen pt-20 pb-16 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {error && (
          <div className="mb-6 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/30 text-red-400">{error}</div>
        )}
                ...
                {loading ? (
                  ...
                  <tr key={i.id} className="border-t border-white/5 hover:bg-white/3 transition-colors">
                    ...
                    <td className="px-4 py-3 text-gray-400 hidden lg:table-cell">
                      {i.stipend ? `₹${i.stipend.toLocaleString()}` : '—'}
                    </td>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center gap-3 mt-6">
            ...
            <span className="flex items-center text-gray-400 text-sm">Page {page + 1} of {totalPages}</span>
        ...
        {deleteId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            <div className="glass-card p-6 max-w-sm mx-4">
              ...
              <p className="text-gray-400 text-sm mb-6">This will permanently delete the internship and all its applications.</p>
              <div className="flex gap-3 justify-end">
                <button onClick={() => setDeleteId(null)} className="btn-outline px-4 py-2 text-sm">Cancel</button>
                <button onClick={() => handleDeleteInternship(deleteId)} className="btn-danger px-4 py-2 text-sm">Delete</button>
              </div>
            </div>
          </div>
        )}
